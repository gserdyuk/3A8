"""Run 45 - assemble the work-crosser prompts for HM44-OA1, one per batch.
Reads run44_raw/HM44-OA1.md section 6; the technology declaration is transcribed below as the
crosser must see it (activities of the declared entries only, from catalogue 1.4); the demanded-work
list rides with the last batch. Writes prompt_<batch>.md into OUT.
Usage: python make_prompts.py <OUT dir>"""
import os
import sys
...
BATCHES = {  # batch: top-level subtrees of S-00
    'A': ['S-01', 'S-02'], 'B': ['S-03'], 'C': ['S-04', 'S-05', 'S-06'],
    'D': ['S-07', 'S-08'], 'E': ['S-10', 'S-11'], 'F': ['S-12', 'S-13'], 'G': ['S-09', 'S-14'],
}

txt = open(os.path.join(HERE, '..', 'run44_raw', 'HM44-OA1.md'), encoding='utf-8').read()
sec6 = txt[txt.index('\n## 6. '):txt.index('\n## 7. ')]
nodes, order = {}, []
for line in sec6.split('\n'):
    ...
    if len(c) != 6 or c[0] == 'id' or set(c[0]) <= set('-'):
        continue
    nodes[c[0]] = dict(name=c[1], parent=c[2], prov=c[3], cov=c[5])
    order.append(c[0])
kids = {}
...
DECLARATION = """# INPUT 2 — the technology declaration (SAS, approved 2026-09-08), as the crossing must see it
...
DEMANDED = """# INPUT 3 — the demanded-work list (`requirements_work.md`, N = 7), with the absorption the declaration records
...
**Partial run.** Scope: the subtrees rooted at **{', '.join(roots)}** of the closed product model `HM44-OA1` — **{n} elements**, listed in INPUT 1 with their parents and own coverage. The model root `S-00` and every other subtree are outside this batch: do not cross them and do not speculate about them. Per the partial-run rule, once-scoped and per-environment activities are **deferred**, not generated, and per-parent activities apply only to parents inside the batch.
...
# INPUT 1 — the product model subtrees in scope (from `HM44-OA1`, `Hotyn-M 1.1`, closed and normalised)

Own coverage only; an element's declared content is its name together with the obligations it covers. `—` = empty own coverage (an aggregate by rule, unless it has no children). Every element listed here has its parent listed here, except the batch roots whose parent is `S-00`.
...
for batch, roots in BATCHES.items():
    ...
    for r in roots:
        ids += subtree(r)
    rows = ''.join(
        f"| {k} | {nodes[k]['name']} | {nodes[k]['parent']} | {nodes[k]['prov']} | {nodes[k]['cov']} |\n"
