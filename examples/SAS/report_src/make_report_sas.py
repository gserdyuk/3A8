...
# the 2.1 chain on the same case (runs 57-59, Hotyn-M 2.1 / W 1.2 / D 2.0, n = 2): raw table sum, band = the two repeats
raw21 = 33444; raw21_lo = 31954; raw21_hi = 34933
# run 60 (Steps C, B, D on the 2.1 reading): calibrated centre band 48 483-52 608, midpoint; the bell's width is the same
# convention as the standing bell - the rho = 0.5 sd of the assembly (mean of the two repeats, 5 288 h) scaled by the calibration
cal21_lo_c, cal21_hi_c = 48483, 52608
cal21 = (cal21_lo_c + cal21_hi_c) / 2
sd21 = 5288 * (cal21 / raw21)
cal21_corr_lo, cal21_corr_hi = 39837, 72031
...
d = {
 ...
 "chart": {
  ...
  "rawMarks": [{"x": pd(raw21), "lo": pd(raw21_lo), "hi": pd(raw21_hi),
                "label": "raw table sum, 2.1 chain %d (runs 57–59, n = 2: %d–%d)" % (pd(raw21), pd(raw21_lo), pd(raw21_hi))}],
  ...
  "bottomUpAlt": [{"id": "2.1 chain, calibrated", "mu": pd(cal21), "sd": pd(sd21), "dash": "6 4"}],
  "outside": [
 ...
 "tiles": [
  ...
  {"cls": "red", "k": "Centre &middot; 2.1 chain", "v": "%d" % pd(cal21), "unit": "pd", "d": "%d net task hours &asymp; 441 staffed person-months &mdash; the dashed bell. The same case through <code>Hotyn-M 2.1</code> / <code>W 1.2</code> / <code>D 2.0</code> (runs 57&ndash;59, raw %d), calibrated by its own gap-blind Step C round (run 60, <b>&times;1.51</b> again). <b>&times;0.88</b> of the standing centre before and after calibration: the whole difference is the shallower skeleton&rsquo;s integration and per-parent items. Calibration spread %d&ndash;%d, not drawn." % (round(cal21), pd(raw21), pd(cal21_corr_lo), pd(cal21_corr_hi))},
